...
void xpd_font_cache(xpd_font_face_t *font, int size, char letter) {
	if (!font->cache[size][letter].loaded) {
		...
		xpd_load_buffer(&font->cache[size][letter].bitmap, font->ftFace->glyph->bitmap.buffer,
...
void xpd_text_draw(xpd_font_face_t *font, std::string text, int x, int y, int size, xpd_text_align_t align,
	...
	for (char character : text) {
		...
		int y_offset = (xpd_font_get_metrics(font, size, character).horiBearingY / 64) - (xpd_font_get_metrics(
			font, size,
			character).height / 64);

		// Fetch & draw texture
		xpd_texture_t *image = xpd_font_get_texture(font, size, character);
		xpd_draw_texture(image, x + (xpd_font_get_metrics(font, size, character).horiBearingX / 64), y + y_offset,
